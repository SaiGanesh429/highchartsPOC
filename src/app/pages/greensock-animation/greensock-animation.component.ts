...
export class GreensockAnimationComponent implements OnInit {
  ...
  liveNews() {
    setTimeout(() => {
      this.infinite = new TimelineMax({ repeat: -1, paused: true });
      const time = 10;
      const elementWidth = this.list.nativeElement.clientWidth + 10;
      this.infinite
        .fromTo(this.list.nativeElement, time,
          { width: elementWidth, rotation: 0.01, x: 0 }, { force3D: true, x: -elementWidth, ease: Linear.easeNone }, 0)
        .fromTo(this.cloned.nativeElement, time,
          { width: elementWidth, rotation: 0.01, x: elementWidth }, { force3D: true, x: 0, ease: Linear.easeNone }, 0)
        .set(this.list.nativeElement, { force3D: true, rotation: 0.01, x: elementWidth })
        .to(this.cloned.nativeElement, time, { force3D: true, rotation: 0.01, x: -elementWidth, ease: Linear.easeNone }, time)
        .to(this.list.nativeElement, time, { force3D: true, rotation: 0.01, x: 0, ease: Linear.easeNone }, time)
